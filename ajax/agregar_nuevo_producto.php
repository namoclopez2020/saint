...
if(isset($_POST['product-categorie'])){
   ...
   $query .=" nombre_producto,codigo_producto,costo_anterior,costo_promedio,costo_actual,precio1,precio2,precio3,categorie_id,fecha_creado,es_serial,id_almacen,impuesto,usa_empaque,medida_paq,medida_und,fraccion";
   $query .=") VALUES (";
   $query .=" '{$nombre_producto}','{$codigo_producto}','{$costo_anterior}','{$costo_promedio}', '{$costo_actual}', '{$precio1}', '{$precio2}', '{$precio3}', '{$id_categoria}', '{$date}','{$es_serial}','{$id_almacen}','{$impuesto}','{$usa_empaque}','{$medida_empaque}','{$medida_unidad}','{$cantidad_empaque}'";
   $query .=")";
   $db->query($query);
